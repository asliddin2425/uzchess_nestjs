...
export class CourseReviewService {
  ...
  async getAll() {
    ...
    const courseReview = plainToInstance(CourseReviewListDto, rawCourseReview, {
  ...
  async getOne(id: number) {
    ...
    return plainToInstance(CourseReviewListDto, rawCourseReview, {
      excludeExtraneousValues: true,
    });
  }

  async create(payload: CourseReviewCreateDto) {
    const newCourseReview = this.repo.create(
      payload as unknown as CourseReview,
    );
    await this.repo.save(newCourseReview);
    return (
      plainToInstance(CourseReviewListDto, newCourseReview),
      {
        excludeExtraneousValues: true,
      }
    );
