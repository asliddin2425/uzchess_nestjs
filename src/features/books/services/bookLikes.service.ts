...
export class BookLikesService {
  ...
  async getAll() {
    ...
    const bookLikes = plainToInstance(BookLikeListDto, rawBookLikes, {
  ...
  async getOne(id: number) {
    ...
    return plainToInstance(BookLikeListDto, rawBookLikes, {
      excludeExtraneousValues: true,
    });
  }
  ...
  async create(payload: BookLikesCreateDto) {
    ...
    return (
      plainToInstance(BookLikeListDto, newBookLikes),
      {
        excludeExtraneousValues: true,
      }
    );
