...
export class BookCategoryControllerPublic {
    constructor(
        private readonly service: BookCategoryPublicService
    ) {}

    @Get()
    @ApiOkResponse({type: () => BookCategoryPublicListDto, isArray: true})
    async findAll(@Req() req: Request): Promise<BookCategoryPublicListDto[]> {
    ...
    @Get(":id")
    @ApiOkResponse({type: () => BookCategoryPublicListDto, isArray: true})
    async findOne(@Param("id", ParseIntPipe) id: number): Promise<BookCategoryPublicListDto> {
