...
export class LanguageControllerPublic {
    constructor(
        private readonly service: LanguagePublicService
    ) {}

    @Get()
    @ApiOkResponse({type: () => LanguagePublicDetailDto, isArray: true})
    async findAll(@Req() req: Request): Promise<LanguagePublicDetailDto[]> {
    ...
    @Get(":id")
    @ApiOkResponse({type: () => LanguagePublicDetailDto, isArray: true})
    async findOne(@Param("id", ParseIntPipe) id: number): Promise<LanguagePublicDetailDto> {
