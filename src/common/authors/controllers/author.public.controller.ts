...
export class AuthorControllerPublic {
    constructor(
        private readonly service: AuthorPublicService
    ) {}

    @Get()
    @ApiOkResponse({type: () => AuthorPublicListDto, isArray: true})
    async findAll(@Req() req: Request): Promise<AuthorPublicListDto[]> {
    ...
    @Get(":id")
    @ApiOkResponse({type: () => AuthorPublicListDto, isArray: true})
    async findOne(@Param("id", ParseIntPipe) id: number): Promise<AuthorPublicListDto> {
